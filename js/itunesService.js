angular.module('itunes').service('itunesService', function($http, $q){
  ...
  //You can return the http request or you can make your own promise in order to manipulate the data before you resolve it.
    ...
    //Code here
    this.returnArtist = function(artist) {
      var defer = $q.defer();
      $http( {
        method: 'JSONP',
        ...
        var newResult = response.data.results;
        var newArray = [];
        for(var i = 0; i < newResult.length; i++) {
          var newArtist = new Word(newResult[i]);
          if(newArtist.Artist !== "Undefined") {
          newArray.push(newArtist);
          }
        }


        return defer.resolve(newArray);
      ...
      var Word = function(artistInfo) {
        ...
        this.Play = artistInfo.trackViewUrl;
